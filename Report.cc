...
{ // if any vector variable added in Antenna_r, need to be added here!
...
{ // to reduce the size of output AraOut.root, remove some information
...
int CircularBuffer::fill(double input_value)
{ // buffer is just filling, don't use last_value
...
int CircularBuffer::numBinsToLatestTrigger()
{
    ...
    for (j = 0; j < N; j++)
    {
        ...
        if (buffer[bin] < 0)
        { // if there's any value here, its because it passed the threshold, so take it
            break;
        }
    } // for j
...
void Report::processArbitraryEvents(
    std::vector<Station_r> &stations,
    int i,
    int j,
    int k,
...
void Report::processCalpulserEvent(
    Event *event,
    std::vector<Station_r> &stations,
    int i,
    int j,
                    ...
                    for (int steps = 1; steps < (int)Ray_Step[ray_sol_cnt][0].size(); steps++)
                    {
                        ...
                        if (dl > 0)
                        {
                            for (int n = 0; n < Nnew / 2; n++)
                            {
                                ...
                                // use ray midpoint for attenuation calculation
                                double IceAttenFactor = (exp(-dl / icemodel->GetFreqDepIceAttenuLength(-Ray_Step[ray_sol_cnt][1][steps], freq_tmp * 1.E-9)) + exp(-dl / icemodel->GetFreqDepIceAttenuLength(-Ray_Step[ray_sol_cnt][1][steps - 1], freq_tmp * 1.E-9))) / 2.; // 1e9 for conversion to GHz
...
int Report::triggerCheckLoop(Settings *settings1, Detector *detector, Event *event, Trigger *trigger, int stationID, int trig_search_init, int max_total_bin, int trig_window_bin, int scan_mode)
{
    ...
    { // initialize Trig_Pass and buffers
    ...
    for (int trig_i = trig_search_init; trig_i < max_total_bin; trig_i++)
    { // scan the different window positions
        ...
        for (int trig_j = 0; trig_j < numChan; trig_j++)
        {
            ...
            { // channel filter: choose if to use lower/borehole channels or not
                ...
                if (settings1->NOISE_CHANNEL_MODE == 0)
                    Pthresh_value[trig_j] = trigger->Full_window[trig_j][trig_i] / (trigger->rmsdiode * detector->GetThresOffset(i, channel_num - 1, settings1));
                if (settings1->NOISE_CHANNEL_MODE == 1)
                    Pthresh_value[trig_j] = trigger->Full_window[trig_j][trig_i] / (trigger->rmsdiode_ch[channel_num - 1] * detector->GetThresOffset(i, channel_num - 1, settings1));
                if (settings1->NOISE_CHANNEL_MODE == 2)
                {
                    ...
                    if (channel_num - 1 < 8)
                        ...
                        Pthresh_value[trig_j] = trigger->Full_window[trig_j][trig_i] / (trigger->rmsdiode_ch[8] * detector->GetThresOffset(i, channel_num - 1, settings1));
                }
        ...
        if (scan_mode > 1 && check_TDR_configuration && window_pass_bit)
        {
            ...
            for (int trig_j = 0; trig_j < numChan; trig_j++)
            { // fill the TDR (unsorted) arrays if they improved...
            ...
            if (settings1->TRIG_MODE == 0)
            { // for N out of all mode

                if (N_pass >= settings1->N_TRIG)
                    for (int ii = 0; ii < N_pass; ii++)
                    { // find the N_pass best channel's TDR and store them.
            ...
            if (settings1->TRIG_MODE == 1)
            { // for N out of either polarization
...
int Report::saveTriggeredEvent(Settings *settings1, Detector *detector, Event *event, Trigger *trigger, int stationID, int trig_search_init, int max_total_bin, int trig_window_bin, int last_trig_bin)
{
    ...
    for (int trig_j = 0; trig_j < numChan; trig_j++)
    {
        ...
        if (stations[i].strings[string_i].antennas[antenna_i].Trig_Pass)
        { // if this channel triggered
            ...
            if (settings1->TRIG_ONLY_LOW_CH_ON == 0)
            {
                ...
                cout << endl
                     << "trigger passed at bin " << stations[i].strings[string_i].antennas[antenna_i].Trig_Pass << "  passed ch : " << trig_j << " (" << detector->stations[i].strings[string_i].antennas[antenna_i].type << "type) Direct dist btw posnu : " << event->Nu_Interaction[0].posnu.Distance(detector->stations[i].strings[string_i].antennas[antenna_i]) << " noiseID : " << stations[i].strings[string_i].antennas[antenna_i].noise_ID[0];
        ...
        for (int mimicbin = 0; mimicbin < waveformLength / 2; mimicbin++)
        {
            ...
            else if (settings1->V_MIMIC_MODE == 1)
            {                                                                                                                                                                                                                                                                                // Global passed bin is the center of the window + delay to each chs from araGeom
            ...
            else if (settings1->V_MIMIC_MODE == 2)
            { // Global passed bin is the center of the window + delay to each chs from araGeom + fitted by eye
    ...
    if (settings1->OUTPUT_TDR_GRAPH > 0)
    {
        ...
        for (int trig_j = 0; trig_j < numChan; trig_j++)
        {
            ...
            // assign Pthresh a value
            if (settings1->NOISE_CHANNEL_MODE == 0)
                thresh_value = detector->GetThres(i, channel_num - 1, settings1) * trigger->rmsdiode * detector->GetThresOffset(i, channel_num - 1, settings1);
            if (settings1->NOISE_CHANNEL_MODE == 1)
                thresh_value = detector->GetThres(i, channel_num - 1, settings1) * trigger->rmsdiode_ch[channel_num - 1] * detector->GetThresOffset(i, channel_num - 1, settings1);
            if (settings1->NOISE_CHANNEL_MODE == 2)
            {
                ...
                if (channel_num - 1 < 8)
                    ...
                    thresh_value = detector->GetThres(i, channel_num - 1, settings1) * trigger->rmsdiode_ch[8] * detector->GetThresOffset(i, channel_num - 1, settings1);
            }
...
void Report::MakeUsefulEvent(Detector *detector, Settings *settings1, Trigger *trigger, int stationID, int stationIndex, UsefulIcrrStationEvent *theUsefulEvent)
{
    if (stationID < detector->params.number_of_stations)
    {
        ...
        for (int ch_loop = 0; ch_loop < ch_limit; ch_loop++)
        {
            ...
            int UsefulEventBin;
            if (settings1->NFOUR / 2 < EFFECTIVE_LAB3_SAMPLES * 2)
                UsefulEventBin = settings1->NFOUR / 2;
            else
                UsefulEventBin = EFFECTIVE_LAB3_SAMPLES * 2;
...
void Report::Select_Wave_Convlv_Exchange(Settings *settings1, Trigger *trigger, Detector *detector, int signalbin0, int signalbin1, int signalbin2, vector<double> &V0, vector<double> &V1, vector<double> &V2, int *noise_ID, int ID, int StationIndex, vector<double> *V_with_noise)
{
    ...
    for (int bin = 0; bin < BINSIZE * 2; bin++)
    {
        ...
        // exchange from pure noise to noise + signal
        if (bin < signal_dbin)
        { // bins where no second signal is shown
            if (signal_dbin0 + bin < BINSIZE)
            { // previous signal is also here!
                V_total_forconvlv[bin] = V_total_forconvlv[bin] + V1[bin] + V0[signal_dbin0 + bin];
                V_tmp[bin] = V1[bin] + V0[signal_dbin0 + bin];
                V_with_noise->push_back(V_total_forconvlv[-1]);
            }
            else
            { // no previous signal, and next signal
                V_total_forconvlv[bin] = V_total_forconvlv[bin] + V1[bin];
                V_tmp[bin] = V1[bin];
                V_with_noise->push_back(V_total_forconvlv[-1]);
            }
...
void Report::ApplyAntFactors(double heff, Vector &n_trg_pokey, Vector &n_trg_slappy, Vector &Pol_vector, int ant_type, double &pol_factor, double &vmmhz, double antenna_theta, double antenna_phi)
...
void Report::ApplyAntFactors_Tdomain(double AntPhase, double heff, Vector &n_trg_pokey, Vector &n_trg_slappy, Vector &Pol_vector, int ant_type, double &pol_factor, double &vm_real, double &vm_img, Settings *settings1, double antenna_theta, double antenna_phi, double freq, double Z_A, bool useInTransmitterMode)
{ // vm is input and output. output will have some antenna factors on it
...
void Report::ApplyAntFactors_Tdomain_FirstTwo(double heff, double heff_lastbin, Vector &n_trg_pokey, Vector &n_trg_slappy, Vector &Pol_vector, int ant_type, double &pol_factor, double &vm_bin0, double &vm_bin1, double antenna_theta, double antenna_phi, double freq, double Z_A, bool useInTransmitterMode)
{ // vm is input and output. output will have some antenna factors on it
...
void Report::InvertAntFactors_Tdomain(double AntPhase, double heff, Vector &Pol_vector, int ant_type, double &pol_factor, double &vm_real, double &vm_img, double antenna_theta, double antenna_phi, bool useInTransmitterMode)
{ // vm is input and output. output will have some antenna factors on it
...
void Report::InvertAntFactors_Tdomain_FirstTwo(double heff, double heff_lastbin, Vector &Pol_vector, int ant_type, double &pol_factor, double &vm_bin0, double &vm_bin1, double antenna_theta, double antenna_phi)
{ // vm is input and output. output will have some antenna factors on it
...
void Report::ApplyFilter(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetFilterGain(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyPreamp(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetPreampGain(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyFOAM(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetFOAMGain(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyFilter_NFOUR(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetFilterGain_NFOUR(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyFilter_OutZero(double freq, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetFilterGain_1D_OutZero(freq)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyElect_Tdomain(double freq, Detector *detector, double &vm_real, double &vm_img, int gain_ch_no, Settings *settings1)
...
void Report::ApplyElect_Tdomain_FirstTwo(double freq0, double freq1, Detector *detector, double &vm_bin0, double &vm_bin1, int gain_ch_no)
{ // read elect chain gain (unitless), phase (rad) and apply to V/m
...
void Report::InvertElect_Tdomain(double freq, Detector *detector, double &vm_real, double &vm_img, int gain_ch_no)
{ // read elect chain gain (unitless), phase (rad) and apply to V/m
...
void Report::InvertElect_Tdomain_FirstTwo(double freq0, double freq1, Detector *detector, double &vm_bin0, double &vm_bin1, int gain_ch_no)
{ // read elect chain gain (unitless), phase (rad) and apply to V/m
...
void Report::ApplyPreamp_NFOUR(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetPreampGain_NFOUR(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyPreamp_OutZero(double freq, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetPreampGain_1D_OutZero(freq)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyFOAM_NFOUR(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetFOAMGain_NFOUR(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyFOAM_OutZero(double freq, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetFOAMGain_1D_OutZero(freq)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyRFCM(int ch, int bin_n, Detector *detector, double &vmmhz, double RFCM_OFFSET)
{ // read RFCM gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetRFCMGain(ch, bin_n) + RFCM_OFFSET) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyFilter_databin(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetFilterGain_databin(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyPreamp_databin(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetPreampGain_databin(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyFOAM_databin(int bin_n, Detector *detector, double &vmmhz)
{ // read filter gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetFOAMGain_databin(bin_n)) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyRFCM_databin(int ch, int bin_n, Detector *detector, double &vmmhz, double RFCM_OFFSET)
{ // read RFCM gain in dB and apply unitless gain to vmmhz

    vmmhz = vmmhz * pow(10., (detector->GetRFCMGain_databin(ch, bin_n) + RFCM_OFFSET) / 20.); // from dB to unitless gain for voltage
}

void Report::ApplyNoiseFig_databin(int ch, int bin_n, Detector *detector, double &vmmhz, Settings *settings1)
{ // read noise figure and apply unitless gain to vmmhz
...
void Report::GetAngleAnt(Vector &rec_vector, Position &antenna, double &ant_theta, double &ant_phi)
{ // ant_theta and ant_phi is in degree
...
void Report::GetNoiseWaveforms(Settings *settings1, Detector *detector, double v_noise, double *vnoise)
{
    ...
    if (settings1->NOISE == 0)
    { // NOISE == 0 : flat thermal noise with Johnson-Nyquist noise
    ...
    { // currently there are no more options!
...
void Report::GetNoiseWaveforms_ch(Settings *settings1, Detector *detector, double v_noise, double *vnoise, int ch)
{
    ...
    else if (settings1->NOISE == 1)
    { // NOISE == 1 : use Rayleigh dist. fits
...
void Report::GetNoisePhase(Settings *settings1)
{
    ...
    {                                                    // noise with DATA_BIN_SIZE bin array
...
void Report::MakeArraysforFFT(Settings *settings1, Detector *detector, int StationIndex, vector<double> &vsignal_array, double *vsignal_forfft)
{
    ...
    if (settings1->AVZ_NORM_FACTOR_MODE == 0)
    { // use previous normalization factors
    ...
    else if (settings1->AVZ_NORM_FACTOR_MODE == 1)
    { // use new (fixed) normalization factors
...
double Report::FindPeak(double *waveform, int n)
{ // same with icemc, trigger-> AntTrigger::FindPeak
        ...
        for (int i = 0; i < detector->params.number_of_stations; i++)
        {
            ...
            for (int j = 0; j < detector->stations[i].strings.size(); j++)
            {
                ...
                for (int k = 0; k < detector->stations[i].strings[j].antennas.size(); k++)
                {
                    ...
                    if (stations[i].strings[j].antennas[k].ray_sol_cnt)
                    {
                        if (stations[i].strings[j].antennas[k].Rank[0] != 0)
                        { // there is non-zero value! and ray_sol_cnt also non-zero!
...
vector<TGraph *> Report::getWaveformVectorVpol(Detector *detector, int station_i, int event_num, int run_num)
{
    ...
    for (int ch = 0; ch < detector->stations[station_i].number_of_antennas; ch++)
    {
        ...
        if (detector->stations[station_i].strings[string_i].antennas[antenna_i].type != 0)
...
vector<TGraph *> Report::getWaveformVectorHpol(Detector *detector, int station_i, int event_num, int run_num)
{
    ...
    for (int ch = 0; ch < detector->stations[station_i].number_of_antennas; ch++)
    {
        ...
        if (detector->stations[station_i].strings[string_i].antennas[antenna_i].type != 1)
...
vector<double> Report::getHitTimesVector(Detector *detector, int station_i, int polarization)
{
    ...
    for (int ch = 0; ch < detector->stations[station_i].number_of_antennas; ch++)
    {
        ...
        if (polarization >= 0 && detector->stations[station_i].strings[string_i].antennas[antenna_i].type != polarization)
...
double Report::getAverageSNR2(int raysolnum, int station_i, int trig_analysis_mode)
{
    ...
    for (int ant_num = first_PAVPol_index; ant_num < first_PAVPol_index + num_ants; ant_num++)
    {
        ...
        if (temp_snr > pa_snr_cap)
            temp_snr = pa_snr_cap; // Cap SNR
    ...
    while (raySolNum < stations[i].strings[0].antennas[8].SignalBin.size())
    {
        ...
        if (avgSnr > 0.5)
        {
            ...
            { // if a randomly selected value is greater than the PA efficiency we calculated, this event triggers
                cout << endl
                     << "PA trigger ~~~ raySolNum: " << raySolNum;
        ...
        if (hasTriggered == true)
        {
            ...
            if (settings1->TRIG_SCAN_MODE > 1)
            { // prepare TDR storage arrays and initialize all values to 0
                TDR_all_sorted_temp = new double[numChan];
                TDR_Vpol_sorted_temp = new double[numChanVpol];
                TDR_Hpol_sorted_temp = new double[numChanHpol];
                for (int trig_j = 0; trig_j < numChan; trig_j++)
            ...
            for (int trig_j = 0; trig_j < numChan; trig_j++)
            { // initialize Trig_Pass and buffers for each channel
                int string_i = detector->getStringfromArbAntID(i, trig_j);
                int antenna_i = detector->getAntennafromArbAntID(i, trig_j);
            ...
            { // scan the different window positions
                ...
                for (int trig_j = 0; trig_j < numChan; trig_j++)
                {
                    ...
                    if (settings1->NOISE_CHANNEL_MODE == 0)
                    {
                        Pthresh_value[trig_j] = (trigger->Full_window[trig_j][trig_i] /
                                                 (trigger->rmsdiode * detector->GetThresOffset(i, channel_num - 1, settings1)));
                    }
                    if (settings1->NOISE_CHANNEL_MODE == 1)
                    {
                        Pthresh_value[trig_j] = (trigger->Full_window[trig_j][trig_i] /
                                                 (trigger->rmsdiode_ch[channel_num - 1] * detector->GetThresOffset(i, channel_num - 1, settings1)));
                    }
                    if (settings1->NOISE_CHANNEL_MODE == 2)
                    {
                        if (channel_num - 1 < 8)
                        ...
                        {
                            Pthresh_value[trig_j] = (trigger->Full_window[trig_j][trig_i] /
                                                     (trigger->rmsdiode_ch[8] * detector->GetThresOffset(i, channel_num - 1, settings1)));
                        }
                    }
                    ...
                    if (Pthresh_value[trig_j] < powerthreshold)
                    {
                        ...
                        // records all the different Pthresh values that caused local trigger.
                        if (settings1->TRIG_SCAN_MODE > 2)
                        { // save PThresh to ant SCT_threshold_pass if its the best or first
                            if (SCTR_cluster_bit[trig_j] == 0)
                            { // if first trigger in cluster
                                stations[i].strings[string_i].antennas[antenna_i].SCT_threshold_pass.push_back(Pthresh_value[trig_j]);
                            }
                ...
                { // if there's a trigger and anything changes in the buffers, restock the TDR arrays
                    ...
                    for (int ii = 0; ii < numChanVpol; ii++)
                    { // find the best channel's TDR and store them.
                    ...
                    for (int ii = 0; ii < numChanHpol; ii++)
                    { // find the best channel's TDR and store them.
            ...
            for (int trig_j = 0; trig_j < numChan; trig_j++)
            {
                ...
                { // loop over raysol numbers
                    ...
                    {
                        dBin = abs(
                            stations[i].strings[string_i].antennas[antenna_i].SignalBin[m] - stations[i].strings[string_i].antennas[antenna_i].Trig_Pass);
                        if (dBin < mindBin)
                        {
                            stations[i].strings[string_i].antennas[antenna_i].Likely_Sol = m; // store the ray sol number which is minimum difference between Trig_Pass bin
                            mindBin = dBin;
